#' Learn a fast greedy equivalence search (FGES) network for k-fold LaGrACE deviation
#'
...
#' @param trainset index of trainset for FGES
#' @return Path to file containing the output from FGES
#' @export

learn_fges_network_crossvalidation <- function(fges_in_file_path, fges_pd, maxCat =5, out_dir,trainset = NULL){
  ...
  s4.5 <- " -maxCat "
  s5 <- "  -o "
  s7 <- paste0("/fges_net_pd_", s4,'_train_set_',toString(trainset),'.txt')

  fges_java_call <- paste0(s1, ssnpa_path, s1_5, fges_in_file_path, s3, s4,s4.5,toString(maxCat), s5, out_dir, s7)

  system(fges_java_call)

  fges_net_file <- paste0(out_dir,"/fges_net_pd_", s4,'_train_set_',toString(trainset),'.txt')
